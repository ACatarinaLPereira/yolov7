...
def get_list(base_folder, pattern='*.png', only_labeled=False):
    list = []
...
class MyConfig(Config):
    ...
    class_names = ["uavs"]  # TODO load from unique config file, and generate yolo configuration
    ...
    num_augmented_images = int(sys.argv[2]) if len(sys.argv) > 2 else 4
    ...
    base_folder = expanduser(sys.argv[1]+'/') if len(sys.argv) > 1 else os.getcwd()+'/' #data_augmentation
...
class MyDataset(utils.Dataset):
    ...
    def compute_test_images_stats():
        ...
        for path in test_labels_path_list:
            with open(path, 'r') as f:
                ...
                classes += list(map(lambda l: int(l.split(' ')[0]), bbs))
                widths += list(map(lambda l: float(l.split(' ')[3]), bbs))
                heights += list(map(lambda l: float(l.split(' ')[4]), bbs))
    ...
    def load_mask(self, image_id):
        ...
        num_object_instances = 1 # Neste caso est'a forcado a ser um objeto mas isso pode ser variavel mas tem de se por a mao sem o ficheiro de legenda
        ...
        mask = np.zeros((image_shape[0], image_shape[1], num_object_instances), dtype=bool)
        class_id_list = []
        #index = 0
        #for current_dict in labels_dicts:
            ...
            #class_name = current_dict['label_class']
                ...
                #index += 1
        mask[:, :, 0] = True
        #class_ids = np.asarray(1, dtype=np.int)
        class_ids = 0
    ...
    def compute_object_patches(self, image, mask, class_ids):
        ...
        for i in range(0, num_objects):
            ...
            print('image', image_patch.shape)
            mask_patch = object_instance[rmin:rmax, cmin:cmax]
            print('mask',image_patch.shape)
            #current_class_id = class_ids[i]
            current_class_id = 0
            current_dict = {"image": image_patch, "mask": mask_patch}
            print('self.object_dict',self.object_dict)
            self.object_dict[current_class_id+1].append(current_dict)
            #self.object_dict[current_class_id] = current_dict
            #print('self.object_dict depois',self.object_dict)
            # visualize_patches(image_patch, mask_patch, self.class_names_dict[current_class_id])
    ...
    def extract_masks_and_image_patches(self): #, pickled_objects_path="object_dict.pkl"):
        ...
        print('object_dict.shape',object_dict)
    ...
    def generate_augmented_image(self, ir_flag):
        ...
        for class_id in selected_class_ids:
            ...
            mask_rgb = np.dstack([mask_patch] * 4)
            #print('mask_rgb', mask_rgb.shape)
